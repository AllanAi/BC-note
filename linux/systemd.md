...
使用systemctl和top命令查询服务占用内存不同 https://blog.csdn.net/wylfengyujiancheng/article/details/107529412
...
> - `ExecStart`：启动当前服务的命令
> - `ExecStartPre`：启动当前服务之前执行的命令，不能是后台长期运行的程序
...
`ExecStart=/usr/bin/sh -c 'exec /usr/bin/my_binary [arguments] >>log1.log 2>>log2.log'`
